import { BarChart, Calculator, CalendarCheck, CirclePlus } from "lucide-react";
import { useState } from "react";
import { motion } from "framer-motion";
import AddWorkLog from "./AddWorkLog";
import CalcuLate from "./CalcuLate";
import PendingPage from "./PendingPage";
import StatusPage from "./StatusPage";

const tabs = [
  { id: "addwork", label: "AddWorkLog", icon: CirclePlus },
  { id: "calculate", label: "Calculate", icon: Calculator },
  { id: "pending", label: "Pending", icon: CalendarCheck },
  { id: "status", label: "Status", icon: BarChart },
];

const EmployeePage = () => {
  const [activeTab, setActiveTab] = useState("addwork");
  return (
    <div className="flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="relative z-10 container mx-auto px-4 py-16">
        <motion.h1
          className="text-4xl font-bold mb-8 text-white text-center"
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
        >
          Employee Dashboard
        </motion.h1>

        <div className="flex justify-center mb-8">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center px-4 py-2 mx-2 rounded-md transition-colors duration-200 ${
                activeTab === tab.id
                  ? "bg-blue-600 text-white"
                  : "bg-gray-700 text-gray-300 hover:bg-gray-600"
              }`}
            >
              <tab.icon className="mr-2 h-5 w-5" />
              {tab.label}
            </button>
          ))}
        </div>

        {activeTab === "addwork" && <AddWorkLog />}
        {activeTab === "calculate" && <CalcuLate />}
        {activeTab === "pending" && <PendingPage />}
        {activeTab === "status" && <StatusPage />}
      </div>
    </div>
  );
};

export default EmployeePage;
